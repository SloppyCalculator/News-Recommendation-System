...
public class RecommendedArticlesController {
    ...
    private final ArticleRepository articleRepository = new ArticleRepository(databaseManager);
    ...
    public void initialize() throws IOException, ParseException, InterruptedException {
        ...
        // convert integer lists to string lists to be easily accessed
        List<String> likedArticles = convertIntListToStringList(
                getArticleIdsFromInteractions(
                        userArticleInteractionRepository.loadUserActionsByAction(user.getUserID(), "liked")));
        List<String> readArticles = convertIntListToStringList(
                getArticleIdsFromInteractions(
                        userArticleInteractionRepository.loadUserActionsByAction(user.getUserID(), "read")));
        List<String> skippedArticles = convertIntListToStringList(
                getArticleIdsFromInteractions(
                        userArticleInteractionRepository.loadUserActionsByAction(user.getUserID(), "skipped")));
        // Get recommendations
        List<String> recommendationsString = engine.getRecommendationsConcurrently(readArticles, likedArticles, skippedArticles);
        // Convert to integer string
        List<Integer> recommendations = convertStringListToIntList(recommendationsString);
        List<Article> recommends = articleRepository.getArticlesByIds(recommendations);
        ...
        if (recommends == null || recommends.isEmpty()) {
            showFeedback("No actions found. Loading default articles.");
            ...
            articlesTable.getItems().addAll(recommends);
        }
        // Prevent resizing of the action column
    ...
    private void addActionButtons() {
        // Set up actionColumn with buttons
        actionColumn.setCellFactory(column -> new TableCell<>() {
            ...
            private final HBox buttonBox = new HBox(5, readButton, likeButton, skipButton);

            {
                ...
                // Set button sizes
                readButton.setMinWidth(30);
                readButton.setMaxWidth(50);
                readButton.setPrefWidth(40);

                likeButton.setMinWidth(30);
                likeButton.setMaxWidth(50);
                likeButton.setPrefWidth(40);

                skipButton.setMinWidth(30);
                skipButton.setMaxWidth(50);
                skipButton.setPrefWidth(40);
    ...
    private void likeArticle(Article article) {
        ...
        // Example: Update in database (optional)
        //articleRepository.updateArticleAsLiked(article);
    }
    ...
    public void removeLike(Article article){
        ...
        tableView.setItems(FXCollections.observableArrayList(currentList));
    ...
    private void openChangeUsernameWindow(User user) {
        ...
        saveButton.setOnAction(event -> {
            ...
            // Replace with actual logic for changing username
    ...
    private static List<String> convertIntListToStringList(List<Integer> intList) {
        return intList.stream()
                .map(String::valueOf) // Convert each integer to a string
                .collect(Collectors.toList()); // Collect the result as a list of strings
    }
    ...
    private static List<Integer> convertStringListToIntList(List<String> stringList) {
        return stringList.stream()
                ...
                .collect(Collectors.toList()); // Collect the result as a list of integers
    }

    private static List<Integer> getArticleIdsFromInteractions(List<UserArticleInteraction> interactions) {
        return interactions.stream()
                .map(UserArticleInteraction::getArticleId) // Extract the articleId from each interaction
